...
void appCreate(int width, int height)
{
	...
	// AppImage texture
	appTexture = sdlLoadImage("ironwail.jpg");
...
void appInitSDL(void) 
{
	...
	sdlWindow = SDL_CreateWindow(" .: AppImage Ironwail Window :. ", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, iWidth, iHeight, ulFlags);
	if (!sdlWindow) {
...
struct GLTexture sdlLoadImage(const char* strFileName) 
{
	...
	strncat(path_with_img, "ironwail.jpg", 12);
